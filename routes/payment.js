...
// for link payment need to add these
...
// Payment notification endpoint without link payment support
router.post('/notify', async (req, res) => {
  ...
  const email = order_id;
  const plan = custom_2;
  const userType = custom_1;
  ...
  console.log('Payment notification for orderid:', order_id);
  ...
  if (local_md5sig === md5sig && status_code == '2') {
      ...
      if (userResult.rows.length > 0) {
        ...
        const updateResult = await client.query(
          `UPDATE user_subscriptions
           SET plan = $1, startDate = $2, endDate = $3, isPaid = true, transactionId = $4, userType = $5, updatedAt = NOW()
           WHERE email = $6
           RETURNING *`,
          [plan, startDate, endDate, payment_id, userType, email]
        );
        ...
        const insertResult = await client.query(
          `INSERT INTO user_subscriptions (email, plan, startDate, endDate, isPaid, transactionId, createdAt, updatedAt, userType, identitymanagedby)
           VALUES ($1, $2, $3, $4, true, $5, NOW(), NOW(), $6, 'Google')
           RETURNING *`,
          [email, plan, startDate, endDate, payment_id, userType]
        );
    ...
    console.log('Payment verification failed for payment id and email:', payment_id, email);
    res.sendStatus(400);
  }
});

// Payment notification endpoint with link payment support
// router.post('/notify', async (req, res) => {
//   console.log('Payment notification received');
...
//         // Insert new user's subscription
//         const insertResult = await client.query(
//           `INSERT INTO user_subscriptions (email, plan, startDate, endDate, isPaid, transactionId, createdAt, updatedAt, userType, identityMangedBy)
//            VALUES ($1, $2, $3, $4, true, $5, NOW(), NOW(), $6, 'Google')
//            RETURNING *`,
//           [email, plan, startDate, endDate, payment_id, userType]
//         );
